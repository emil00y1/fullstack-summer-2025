...
export async function fetchPostsWithLikes(params = {}, userId = null) {
  try {
    ...
    return posts.map((post, index) => {
      ...
      let uniqueId;
      if (post.type === "repost") {
        // For reposts, combine post ID + repost ID + repost user ID to ensure uniqueness
        // This handles cases where the same post is reposted by multiple users
        uniqueId = `repost-${post.id}-${post.repostId}-${post.repostUserId}`;
      } else {
        // For original posts, use the post ID with a prefix
        uniqueId = `post-${post.id}`;
      }

      // Add index as fallback to ensure absolute uniqueness in edge cases
      const finalUniqueId = `${uniqueId}-${offset + index}`;

      return {
        ...post,
        encryptedId: encryptId(post.id),
        uniqueKey: encryptId(finalUniqueId), // Now guaranteed to be unique
        likes: likeUserIds,
        isLiked: userId ? likeUserIds.includes(userId) : false,
        reposts: repostUserIds,
        isReposted: userId ? repostUserIds.includes(userId) : false,
